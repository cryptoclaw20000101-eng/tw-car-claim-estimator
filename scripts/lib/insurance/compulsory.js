...
function calcEmergency(input) {
    return mkItem('emergencyFee', '急救費用', input.emergencyFee, input.emergencyFee, null, input.emergencyFee === 0 ? '未輸入急救費' : null, input.emergencyFee === 0 ? '請補急診收據' : null);
}
function calcAmbulance(input) {
    return mkItem('ambulanceFee', '救護車費', input.ambulanceFee, input.ambulanceFee, null, input.ambulanceFee === 0 ? '未輸入救護車費' : null, input.ambulanceFee === 0 ? '請補救護車收據' : null);
}
function calcNhiCopayment(input) {
    // 健保自付額：依實際收據實支實付，無上限
    return mkItem('nhiCopayment', '健保診療自負額', input.nhiCopayment, input.nhiCopayment, null, input.nhiCopayment === 0 ? '未輸入健保自付額' : null, input.nhiCopayment === 0 ? '請補健保署醫療費用收據' : null);
}
function calcRegistration(input) {
    return mkItem('registrationFee', '掛號費', input.registrationFee, input.registrationFee, null, input.registrationFee === 0 ? '未輸入掛號費' : null, input.registrationFee === 0 ? '請補門診收據' : null);
}
function calcDiagnosisCertificate(input) {
    return mkItem('diagnosisCertificateFee', '診斷證明書費', input.diagnosisCertificateFee, input.diagnosisCertificateFee, null, input.diagnosisCertificateFee === 0 ? '未輸入診斷書費' : null, input.diagnosisCertificateFee === 0 ? '請補醫院開立之診斷證明書（強制險必備文件）' : null);
}
function calcNonNhi(input) {
    return mkItem('nonNhiNecessaryMedicalFee', '非健保必要醫療費', input.nonNhiNecessaryMedicalFee, input.nonNhiNecessaryMedicalFee, null, input.nonNhiNecessaryMedicalFee === 0 ? '未輸入非健保自費項目' : null, input.nonNhiNecessaryMedicalFee === 0
...
function calcMealFee(input) {
    ...
    const reduction = approved < input.mealFee
...
function calcProsthesis(input) {
    ...
    const reduction = approved < input.prosthesisFee
        ? '義肢費超出單肢 50,000 上限'
...
function calcDenture(input) {
    ...
    const reduction = approved < input.dentureFee
        ? `義齒費超出 ${input.missingTeethCount} 齒上限（每齒 10,000，最多 50,000）`
        : input.dentureFee === 0
            ? '未輸入義齒費'
            : null;
    const hint = input.dentureFee > 0 && input.missingTeethCount === 0
        ? '需補缺牙數量資料'
        : input.dentureFee === 0
            ? '請補義齒收據 + 缺牙數量（影響上限計算）'
            : null;
...
function calcArtificialEye(input) {
    ...
    const reduction = approved < input.artificialEyeFee
        ? '義眼費超出每顆 10,000 上限'
        : input.artificialEyeFee === 0
            ? '未輸入義眼費'
            : null;
    const hint = input.artificialEyeFee === 0 ? '請補義眼裝配證明' : null;
    return mkItem('artificialEyeFee', '義眼費', input.artificialEyeFee, approved, cap, reduction, hint);
}
function calcMedicalMaterial(input) {
    ...
    if (diff !== 0) {
        if (special >= assistive)
            approvedSpecial += diff;
        else
            approvedAssistive += diff;
    }
    const reduction = approvedTotal < subtotal
        ? `特殊材料＋輔具合計 ${subtotal.toLocaleString()} 元超出 2 萬上限，採申請比例分攤`
        : null;
    const hint = special > 0 && special > 15000
...
function calcTransportation(input) {
    ...
    return mkItem('transportationFee', '接送費用', input.transportationFee, approved, cap, reduction, null);
}
function calcNursing(input) {
    ...
    const reduction = approved < input.nursingFee ? `看護費超出 1,200 × ${eligibleDays} 日上限（最高 30 日）` : null;
...
function calcOther(input) {
    return mkItem('otherNecessaryMedicalFee', '其他必要醫療費', input.otherNecessaryMedicalFee, input.otherNecessaryMedicalFee, null, null, input.otherNecessaryMedicalFee > 0 ? '需檢附醫師必要性證明與收據' : null);
}
// --- 主計算 ----------------------------------------------------------
...
function calcMedicalMaterialOldLaw(input) {
    ...
    if (diff !== 0) {
        ...
        if (items[0].ref === 's')
            approvedSpecial += diff;
        else if (items[0].ref === 'g')
            approvedGeneral += diff;
        else
            approvedAssistive += diff;
    }
    const reduction = approvedTotal < subtotal

...
const KEYWORDS = {
  ...
  // 5 年內 = 2021-2026 民事 + 車禍相關（道路交通事故損害賠償）
  // 排除刑庭（已 isCivilCase 過濾）
...
function absorbCookies(jar, res) {
  var _a, _b
  const setCookie =
    ((_b = (_a = res.headers).getSetCookie) === null || _b === void 0 ? void 0 : _b.call(_a)) ||
...
async function getHtmlWithRetry(jar, url, referer, config) {
  ...
  throw lastErr !== null && lastErr !== void 0
    ? lastErr
    : new Error(`GET ${url} failed after ${maxRetries} retries`)
}
...
function writePrecedent(p) {
  ...
  if (!(0, node_fs_1.existsSync)(outDir)) (0, node_fs_1.mkdirSync)(outDir, { recursive: true })
...
async function main() {
  ...
  if (retryArgIdx >= 0) {
    const n = parseInt(
      (_a = process.argv[retryArgIdx + 1]) !== null && _a !== void 0 ? _a : '3',
      10,
    )
  ...
  if (retryDelayArgIdx >= 0) {
    const ms = parseInt(
      (_b = process.argv[retryDelayArgIdx + 1]) !== null && _b !== void 0 ? _b : '500',
      10,
    )
  ...
  for (const { chain, keyword: kw } of jobs) {
    ...
    for (const hit of hits) {
      ...
      try {
        ...
        const yearInt = parseInt(
          ((_e = hit.caseNo.match(/(\d+)/)) === null || _e === void 0 ? void 0 : _e[1]) || '0',
          10,
        )
...
if (
  (_a = process.argv[1]) === null || _a === void 0 ? void 0 : _a.endsWith('scrape-judgments.js')
) {
  main().catch((e) => {

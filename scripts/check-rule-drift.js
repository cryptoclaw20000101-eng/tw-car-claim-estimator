...
console.log('\n📊 HTML 內常數：')
console.log(
  `   COMPULSORY_LIMITS.TOTAL_MEDICAL_CAP = ${htmlConstants.COMPULSORY_LIMITS.TOTAL_MEDICAL_CAP.toLocaleString()}`,
)
...
function grepConstFromTs(tsFile, constName) {
  try {
    ...
    const re = new RegExp(
      `export\\s+const\\s+${constName}\\b[^=]*=\\s*([\\s\\S]+?)\\s*as\\s+const`,
      'm',
    )
...
const checksVal = [
  {
    name: '強制險總額上限',
    html: htmlConstants.COMPULSORY_LIMITS.TOTAL_MEDICAL_CAP,
    expected: 200_000,
    ts: 'compulsory.ts',
  },
  ...
  {
    name: '失能等級 1（最高）',
    html: htmlConstants.DISABILITY_TABLE[1],
    expected: 3_000_000,
    ts: 'disability-tables.ts (新制 2026-07-01)',
  },
  {
    name: '失能等級 15（最低）',
    html: htmlConstants.DISABILITY_TABLE[15],
    expected: 80_000,
    ts: 'disability-tables.ts',
  },
  {
    name: '臺北地區係數',
    html: htmlConstants.REGION_MULTIPLIER['臺北市'],
    expected: 1.15,
    ts: 'region-adjustments.ts',
  },
]
...
checksVal.forEach(({ name, html: h, expected, ts }) => {
  ...
  if (drift < 0.001) {
    console.log(
      `   ✅ ${name.padEnd(15)} HTML=${h.toLocaleString()} == 預期=${expected.toLocaleString()}`,
    )
  } else if (drift < 0.05) {
    console.log(
      `   🟡 ${name.padEnd(15)} HTML=${h.toLocaleString()} vs 預期=${expected.toLocaleString()} 差異 ${(drift * 100).toFixed(1)}%`,
    )
  } else {
    console.log(
      `   ❌ ${name.padEnd(15)} HTML=${h.toLocaleString()} vs 預期=${expected.toLocaleString()} 差異 ${(drift * 100).toFixed(1)}%`,

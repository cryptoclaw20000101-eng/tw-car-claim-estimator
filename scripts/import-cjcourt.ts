...
async function main() {
  ...
  const chain = (args.chain as string) || 'mental_distress'
  const year = args.year ? parseInt(args.year as string, 10) : undefined
  ...
  const dataPath = join(process.cwd(), 'data', 'precedents', fileName)
  if (!existsSync(dataPath)) {
    console.error(`[import-cjcourt] 找不到 ${dataPath}`)
    process.exit(1)
  }
  const rows: PrecedentRow[] = JSON.parse(readFileSync(dataPath, 'utf-8'))
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    if (r.ok && r.amount) {
      ...
      console.log(`  [${i + 1}/${filtered.length}] ✓ ${row.caseNo?.slice(0, 20)}... ${r.amount} 元`)
    } else {
      fail++
      console.log(
        `  [${i + 1}/${filtered.length}] ✗ ${row.caseNo?.slice(0, 20)}... ${r.error || 'no amount'}`,
      )
    }
  }

  if (!dryRun) {
    writeFileSync(dataPath, JSON.stringify(rows, null, 2), 'utf-8')

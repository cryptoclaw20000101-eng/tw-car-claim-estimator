...
async function getHtml(jar: Jar, url: string, referer?: string): Promise<string> {
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (res.status >= 500 && i < 2) { await sleep(500 * 2 ** i); continue; }
...
async function resolveCourtName(sample: Sample, kw: string = "車禍 侵權行為"): Promise<string | null> {
  const jar = newJar();
  ...
  if (!qHash) throw new Error("no qHash");
  ...
  for (const l of links) {
    ...
    if (parts[0] === sample.code && parts[1] === sample.year && parts[2] === sample.caseType && parts[3] === sample.caseNum) {
      target = l.id;
      break;
    }
  }
  if (!target) {
    for (const l of links) {
      if (l.id.split(",")[0] === sample.code) { target = l.id; break; }
    }
  }
  if (!target) return null;
  const detail = await getHtml(
    jar,
    `${BASE}/FJUD/data.aspx?ty=JD&id=${encodeURIComponent(target)}&ot=in`,
...
(async () => {
  ...
  const files = readdirSync(DATA).filter(f => f.endsWith(".json") && !f.startsWith("_") && f !== "precedents-report.html");
  ...
  interface UnkEntry { sample: Sample; keywords: string[]; }
  const unknownMap = new Map<string, UnkEntry>();
  for (const f of files) {
    ...
    for (const p of data) {
      ...
      const code = m?.[1];
      if (code) {
        const yearMatch = p.caseNo?.match(/(\d+)\s*年度\s*(\S+)\s*字第\s*(\S+)\s*號/);
        if (!yearMatch) continue;
        if (!unknownMap.has(code)) {
          unknownMap.set(code, {
            sample: { code, year: yearMatch[1], caseType: yearMatch[2], caseNum: yearMatch[3], caseNo: p.caseNo, chain: chainName },
  ...
  const outFile = join(DATA, "_court-resolution.json");
  ...
  for (const [code, { sample, keywords }] of unknownMap) {
    ...
    writeFileSync(outFile, JSON.stringify(courtMap, null, 2));
    await sleep(300);
  }

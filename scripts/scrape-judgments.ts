  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        if (!quiet)
          console.log(
            `[scrape]   ⚠ ${err.message.slice(0, 60)} 第 ${attempt + 1}/${maxRetries} 次重試...`,
          )
        await new Promise((r) => setTimeout(r, baseDelayMs * 2 ** attempt))
      }
    }
...
async function main() {
  ...
  const chainFilter: ChainKey | null =
    chainArgIdx >= 0 ? (process.argv[chainArgIdx + 1] as ChainKey) : null
  ...
  const delayBetweenSec =
    delayBetweenArgIdx >= 0 ? parseInt(process.argv[delayBetweenArgIdx + 1] ?? '0', 10) : 0
  ...
  const yearMin: number | null =
    yearMinArgIdx >= 0 ? parseInt(process.argv[yearMinArgIdx + 1] ?? '0', 10) : null
  const yearMax: number | null =
    yearMaxArgIdx >= 0 ? parseInt(process.argv[yearMaxArgIdx + 1] ?? '0', 10) : null
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    for (const hit of hits) {
      ...
      try {
        ...
        if (
          !isInYearRange(hit.caseNo, hasYearFilter ? yearMin : null, hasYearFilter ? yearMax : null)
        ) {
        ...
        if (isDryRun) {
          console.log(
            `[scrape]       🧪 [dry-run] ${CHAIN_LABEL[chain]} ${amts.amount.toLocaleString()} 元`,
          )
        } else {
  ...
  if (runAll) console.log(`[scrape] 🌐 全自動模式 — 已跑完所有 chain`)
  ...
  if (autoDeploy && !isDryRun) {
    ...
    try {
      ...
      if (status.trim()) {
        execSync(`git commit -m "chore(dx): v0.27.8 scrape 全部 chain 跑完 (+${totalScraped})"`, {
          stdio: 'inherit',
        })
        // 2. tag
        ...
        execSync(
          'railway up --service 31759164-7453-4a51-a0ff-987cddb9764e --detach -m "force rebuild v0.27.8 scrape all"',
          { stdio: 'inherit' },
        )
      ...
      console.error('[scrape] ✗ auto-deploy 失敗:', (e as Error).message)
    }
  }
}

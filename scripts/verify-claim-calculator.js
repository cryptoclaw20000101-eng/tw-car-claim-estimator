...
const removePatterns = [
  /const \$ = .*$/gm, /const num = .*$/gm, /const bool = .*$/gm,
  /const fmt = .*$/gm, /const fmtRange = .*$/gm,
  /function collectInput[\s\S]*?(?=\nfunction |\nconsole)/m,
  /function goStep[\s\S]*?(?=\nfunction |\nconsole)/m,
  /function renderResults[\s\S]*?(?=\nfunction |\nconsole)/m,
  /function exportPDF[\s\S]*?(?=\nfunction |\nconsole)/m,
  /document\.[^;]+;/g, /window\.[^;]+;/g, /window\.scrollTo[^;]+;/g
];
...
for (let i = funcStartIdx; i < js.length; i++) {
  ...
  else if (js[i] === '}') { brace--; if (started && brace === 0) { pureEnd = i + 1; break; } }
}
...
try {
  ...
  const demo = {
    ...
    actualLeaveDays: 45,
    hasSalaryTransferRecord: true, hasLeaveCertificate: true, hasSalaryDeductionProof: true,
    diagnosisText: '右膝韌帶撕裂',
    hospitalizationDays: 5,
    outpatientVisitCount: 8,
    rehabilitationCount: 12,
    hasSurgery: true, hasRehabilitation: true,
    requiresNursingCare: true, nursingDays: 14,
    hasDisabilityCertificate: false,
    disabilityCategory: '11_lower_limb',
    romLossDegree: 30,
    hasFracture: false, hasNerveDamage: false, hasPermanentImpairment: false,
    requiresNursingCare: true,
    hasScar: false,
    // 給 findRelatedPracticeCases 用的車輛欄位
    vehicleRepairEstimate: 35000, vehicleRepairInvoice: 0,
    emergencyFee: 3500, ambulanceFee: 1200, nhiCopayment: 8500,
    registrationFee: 800, diagnosisCertificateFee: 2500, nonNhiNecessaryMedicalFee: 0,
    wardFeeDays: 5, wardFeeDifference: 6000, mealDays: 5,
    specialMaterialFee: 15000, assistiveDeviceFee: 5000, transportationFee: 3000,
    nursingFee: 16800, prosthesisFee: 0, otherNecessaryMedicalFee: 0,
    vehicleRepairEstimate: 35000, vehicleRepairInvoice: 0,
    vehicleMarketValueBeforeAccident: 80000, salvageValue: 0,
    towingFee: 2000, helmetDamage: 3000, clothingDamage: 1500
  };

  // 模擬真實 input 結構（HTML 內 collectInput 會建好這個 shape）
  const realInput = {
    basics: { accidentCity: demo.accidentCity, accidentDate: '2026-05-15', hasPolicePreliminaryReport: demo.hasPolicePreliminaryReport, hasAccidentAppraisal: demo.hasAccidentAppraisal },
    fault: { otherFaultRatio: demo.otherFaultRatio },
    person: { sixMonthAverageSalary: demo.sixMonthAverageSalary },
    medical: { requiresNursingCare: demo.requiresNursingCare, outpatientVisitCount: demo.outpatientVisitCount }
  };
  // property 補到 input 結構
  realInput.vehicleRepairEstimate = demo.vehicleRepairEstimate;
  // 補 collectInput 結構裡的其他欄位（HTML 內會有，這裡模擬空值）
  realInput.medicalReceipts = { registrationFee: demo.registrationFee, wardFeeDifference: demo.wardFeeDifference, nursingFee: demo.nursingFee, nursingDays: demo.nursingDays, vehicleRepairInvoice: demo.vehicleRepairInvoice, vehicleRepairEstimate: demo.vehicleRepairEstimate };
  ...
  console.log('🏥 強制險 15 細項：', comp.items.length, '項, 醫療 approved =', comp.approved.toLocaleString());
  const matItem = comp.items.find(i => i.key === 'specialMaterialAndAssistive');
  if (matItem && matItem.subItems) {
    console.log('   - 特殊材料+輔具 subItems:', matItem.subItems.length, '子項',
      '(special=' + matItem.subItems[0].applied.toLocaleString() + ' + assistive=' + matItem.subItems[1].applied.toLocaleString() + ')');
  }
  console.log('🦴 失能初篩：', disab.screening, '級 | 等級', disab.possibleLevel, '| 金額', disab.possibleAmount.toLocaleString());
  console.log('   線索：', disab.signals.join(', ') || '無');
  if (disab.needsSupplement.length > 0) console.log('   需補：', disab.needsSupplement.join(', '));
  console.log('💰 精神慰撫金：', civil.painAndSuffering.low.toLocaleString(), '~', civil.painAndSuffering.mid.toLocaleString(), '~', civil.painAndSuffering.high.toLocaleString());
  console.log('💼 工作損失：', civil.workLoss.toLocaleString(), '（證據', civil.workLossEvidence, '）');
  console.log('   短期:', civil.workLossShortTerm.toLocaleString(), '| 長期:', civil.workLossLongTerm.toLocaleString());
  console.log('👨‍⚕️ 看護費：', civil.nursingFeeLow.toLocaleString(), '~', civil.nursingFeeHigh.toLocaleString(), '（', civil.nursingDays, '日）');
  console.log('🚗 第三人責任險：', tp.estimate.low.toLocaleString(), '~', tp.estimate.mid.toLocaleString(), '~', tp.estimate.high.toLocaleString());
  ...
  cases.forEach(c => console.log('   -', c.caseNo, '(', c.court, ',', c.chain, ',', c.similarity + '% )'));
  console.log('\n📊 霍夫曼速查（r=5%）：');
  console.log('   5年:', api.HOFFMANN[5].toFixed(4), '| 10年:', api.HOFFMANN[10].toFixed(4), '| 30年:', api.HOFFMANN[30].toFixed(4));
  console.log('\n💎 強制險合計：', total.toLocaleString());
  console.log('   (醫療', comp.approved.toLocaleString(), '+ 失能', disab.possibleAmount.toLocaleString() + ')');

...
- **2026-07-01 是強制險 §2.3.6 死亡 / 失能給付金額上限提高的施行日（200 萬 → 300 萬）**
- **醫材費拆 subItems / pro-rata 邏輯（特殊材料費 + 輔具費各自 2 萬上限）這部分不是 §2.3.6 新法內容**，是先前已存在的計算邏輯（v0.2.5 / v0.6.6 實作）
- v0.8.2 補上 `isNewLaw(accidentDate)` 切換，確保：
  - **事故日 >= 2026-07-01** → 新法（死亡 / 失能給付 300 萬上限）
  - **事故日 < 2026-07-01** → 舊法（死亡 / 失能給付 200 萬上限）
...
| 事故日                        | 死亡 / 失能給付上限           |
| ----------------------------- | ----------------------------- |
| `null` / `undefined` / `''`   | **新法 300 萬**（保守預設）   |
| `2026-07-01` ~ 至今           | 新法 300 萬                   |
| `2026-06-30` 及之前           | 舊法 200 萬                   |
| 非標準格式（"2026/07/01" 等） | 舊法 200 萬（保守，避免誤判） |
...
- **§2.3.6 真正變更**：死亡給付 200 萬 → 300 萬、失能給付 200 萬 → 300 萬
- **§2.3.6 不變更**：特殊材料費 + 輔具費 + 醫材 pro-rata 拆 subItems（這是先前已存在的計算邏輯）
- **目前程式覆蓋**：v0.24.0+ 只算醫療費用（TOTAL_MEDICAL_CAP = 200_000）+ 失能等級初篩（disability.possibleLevel）；死亡/失能給付金額上限（300 萬）需 user 指定才實作 UI 顯示

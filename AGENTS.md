...
- **不擅自填法院代碼** — 司法院 COURT_CODE 對照表由律師/工程師查證後才填，scrape 端用 `(未知代碼)` 標記
...
- 6 大引擎（強制 / 失能 / 民事 / 第三人 / 地區 / 補件）可借鑑 Ensemble 概念（XGBoost + 規則引擎 + 律師複核）

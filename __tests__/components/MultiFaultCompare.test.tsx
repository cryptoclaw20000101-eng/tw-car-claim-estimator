...
describe('MultiFaultCompare', () => {
  it('renders 3 scenarios (30/70, 50/50, 70/30)', () => {
    const html = renderToString(
      <MultiFaultCompare
        ...
        bodilyInjuryAmount={60000}
        propertyDamageAmount={40000}
      />,
    )
  ...
  it('calculates each scenario with correct ratio', () => {
    // civilMidBaseline 100000 + body 60K + property 40K = 100K total
  ...
  it('handles zero baseline (no claim)', () => {
    const html = renderToString(
      <MultiFaultCompare civilMidBaseline={0} bodilyInjuryAmount={0} propertyDamageAmount={0} />,
    )
    // 即使 0 也要 render 3 scenarios
    expect(html).toMatch(/己方.{0,20}30.{0,20}%.{0,20}對方.{0,20}70/)
    expect(html).toContain('<!-- -->0<!-- -->') // $0 in 注釋裡
  })

  it('handles defaults (only civilMidBaseline provided)', () => {
